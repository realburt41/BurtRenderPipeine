// Isolated candidate: not enabled by the BRP runtime. See NativeFineOccupancy442.
Shader "Hidden/Codex/Fine Voxelize 442"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            RWTexture3D<uint> _FineLow : register(u5);
            RWTexture3D<uint> _FineHigh : register(u6);
            ...
            float4 BurtGIVoxelizeFragment(VoxelFragment input) : SV_Target
            {
                ...
                uint3 fineCoord = min((uint3)(uvw * (float3)(volumeSize * 4u)), volumeSize * 4u - 1u);
                uint3 localFine = fineCoord & 3u;
                uint fineBit = localFine.x + localFine.y * 4u + localFine.z * 16u;
                // The caller validates ((primitiveBase + count)*3)*64 < UINT_MAX.
                // At 4N raster resolution the old primitive/axis owner alone
                // is no longer unique per material cell. Include the fine slot
                // so only one fragment can resolve each coarse float4 payload.
                uint ownerKey = input.ownerKey * 64u + fineBit;
                if (_BurtGIVoxelizeResolvePhase == 0)
                {
                    // All opaque fragments contribute geometry, not just the
                    // elected coarse material winner. RGB has no validity role.
                    if (fineBit < 32u) InterlockedOr(_FineLow[voxelCoord], 1u << fineBit);
                    else InterlockedOr(_FineHigh[voxelCoord], 1u << (fineBit & 31u));
                    InterlockedMin(_BurtGIVoxelizeOwnerTexture[voxelCoord], ownerKey);
                    return 0.0;
                }
                if (_BurtGIVoxelizeOwnerTexture[voxelCoord] != ownerKey) discard;
