...
namespace Burt.RenderPipeline // 让阴影数据和渲染请求、灯光数据保持在同一个 BurtRP 运行时命名空间里。
{
    public sealed class BurtShadowData // 保存一次 BurtRenderRequest 内主光阴影需要的全部运行时参数。
    {
        ...
        private void ApplyVolumeSettings(BurtMainLightShadowVolumeComponent volumeSettings) // ??? VolumeStack ?????????????????????
        {
    ...
    internal static class BurtShadowUtility // 定义阴影渲染辅助工具，集中处理主光阴影数据读取、合并和常用派生参数。
    {
        ...
        public static BurtMainLightShadowVolumeComponent ResolveMainLightShadowVolumeComponent() // ??? VolumeStack ?? BurtRP ?????????
        {
            ...
            return stack.GetComponent<BurtMainLightShadowVolumeComponent>(); // ?????? Unity ?????/???????? overrideState ???????
        }
