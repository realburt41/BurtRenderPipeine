...
{
    public readonly struct BurtAutoExposureDebugSnapshot
    {
        public readonly bool HasState;
        public readonly BurtExposureMode Mode;
        ...
        public BurtAutoExposureDebugSnapshot(
            bool hasState,
    ...
    public static class BurtAutoExposureDebugUtility
    {
        public static bool TryGetSnapshot(Camera camera, out BurtAutoExposureDebugSnapshot snapshot)
        {
            return BurtAutoExposureUtility.TryGetDebugSnapshot(camera, out snapshot);
        }
    }

    internal static class BurtAutoExposureUtility
    {
        ...
        private const string SampleRejectedReasonNone = BurtPhysicalExposureSettings.DefaultAutoSampleRejectedReason;
        ...
        private sealed class CameraExposureState
        {
            ...
            public float CurrentEV100 = BurtPhysicalExposureSettings.DefaultManualEv100;
            public float TargetEV100 = BurtPhysicalExposureSettings.DefaultAutoTargetEv100;
            public float AverageLuminance = BurtPhysicalExposureSettings.DefaultAutoAverageLuminance;
            public float AverageLogLuminance = BurtPhysicalExposureSettings.DefaultAutoAverageLogLuminance;
            public float MinEV100 = BurtPhysicalExposureSettings.DefaultAutoMinEv100;
            public float MaxEV100 = BurtPhysicalExposureSettings.DefaultAutoMaxEv100;
            public float MiddleGrey = BurtPhysicalExposureSettings.DefaultAutoMiddleGrey;
            public float LowPercent = BurtPhysicalExposureSettings.DefaultAutoLowPercent;
            public float HighPercent = BurtPhysicalExposureSettings.DefaultAutoHighPercent;
            public float HistogramMinEV100 = BurtPhysicalExposureSettings.DefaultAutoHistogramMinEv100;
            public float HistogramMaxEV100 = BurtPhysicalExposureSettings.DefaultAutoHistogramMaxEv100;
            public BurtExposureMode Mode = BurtPhysicalExposureSettings.DefaultMode;
        ...
        public static BurtPhysicalExposureSettings ResolveSettings(
            Camera camera,
            BurtExposureVolumeComponent exposure)
        {
            ...
            var baseSettings = CreateSettings(exposure, BurtPhysicalExposureSettings.DefaultManualEv100, false, false);
            ...
            if (state == null)
            {
                return CreateSettings(exposure, BurtPhysicalExposureSettings.DefaultManualEv100, false, false);
            }
        ...
        public static bool ShouldCapture(BurtPhysicalExposureSettings settings, Camera camera)
        {
            return camera != null &&
        ...
        public static void CaptureAverageLogLuminance(
            ...
            BurtPhysicalExposureSettings settings,
            Material material,
            int[] textureIds,
            ...
            int texelSizeId)
        {
            ...
            if (settings.Mode == BurtExposureMode.AutomaticHistogram)
            {
        ...
        public static BurtPhysicalExposureSettings UpdateAfterCapture(Camera camera, BurtExposureVolumeComponent exposure, float deltaTime)
        {
            ...
            if (state.HasSample)
            {
                ...
                var speed = targetEV100 > state.CurrentEV100
                    ? Mathf.Clamp(SanitizeFinite(exposure.autoSpeedUp.value, BurtPhysicalExposureSettings.DefaultAutoSpeedUp), 0.02f, 20f)
                    : Mathf.Clamp(SanitizeFinite(exposure.autoSpeedDown.value, BurtPhysicalExposureSettings.DefaultAutoSpeedDown), 0.02f, 20f);
        ...
        public static BurtPhysicalExposureSettings GetDebugSettings(Camera camera, BurtExposureVolumeComponent exposure)
        {
            return ResolveSettings(camera, exposure);
        }

        internal static bool TryGetDebugSnapshot(Camera camera, out BurtAutoExposureDebugSnapshot snapshot)
        {
            snapshot = default;
            if (camera == null)
            {
            ...
            RecoverTimedOutReadback(state);
            snapshot = new BurtAutoExposureDebugSnapshot(
                true,
                state.Mode,
                state.CurrentEV100,
                state.TargetEV100,
        ...
        private static BurtPhysicalExposureSettings CreateSettings(
            BurtExposureVolumeComponent exposure,
            float autoEV100,
            bool autoHasSample,
            bool autoReadbackPending,
            float autoAverageLuminance = BurtPhysicalExposureSettings.DefaultAutoAverageLuminance,
            float autoAverageLogLuminance = BurtPhysicalExposureSettings.DefaultAutoAverageLogLuminance,
            float autoTargetEV100 = BurtPhysicalExposureSettings.DefaultAutoTargetEv100,
            int autoReadbackAgeFrames = BurtPhysicalExposureSettings.DefaultAutoFrameAge,
            int autoSampleAgeFrames = BurtPhysicalExposureSettings.DefaultAutoFrameAge,
            int autoSampleCount = BurtPhysicalExposureSettings.DefaultAutoSampleCount,
            string autoSampleRejectedReason = BurtPhysicalExposureSettings.DefaultAutoSampleRejectedReason)
        {
        ...
        private static void CompleteReadback(int cameraId, BurtExposureMode mode, AsyncGPUReadbackRequest request)
        {
            if (!CameraStates.TryGetValue(cameraId, out var state))
            {
                return;
            ...
            var averageLogLuminance = mode == BurtExposureMode.AutomaticHistogram
                ? CalculateHistogramAverageLogLuminance(state, data)
                : Mathf.Clamp(SanitizeFinite(data[0], BurtPhysicalExposureSettings.DefaultAutoAverageLogLuminance), LogLuminanceMin, LogLuminanceMax);
            ...
            CameraExposureState state)
        {
            ...
            state.ReadbackPending = true;
            state.ReadbackRequestedFrame = Time.frameCount;
            cmd.RequestAsyncReadback(state.ReadbackTexture, request => CompleteReadback(camera.GetInstanceID(), BurtExposureMode.AutomaticHistogram, request));
        }

        private static void CacheAutoParameters(CameraExposureState state, BurtExposureVolumeComponent exposure)
        {
            ...
            var minEV100 = Mathf.Clamp(SanitizeFinite(exposure.autoMinEV100.value, BurtPhysicalExposureSettings.DefaultAutoMinEv100), -16f, 24f);
            var maxEV100 = Mathf.Clamp(SanitizeFinite(exposure.autoMaxEV100.value, BurtPhysicalExposureSettings.DefaultAutoMaxEv100), -16f, 24f);
            ...
            state.MiddleGrey = Mathf.Clamp(SanitizeFinite(exposure.autoMiddleGrey.value, BurtPhysicalExposureSettings.DefaultAutoMiddleGrey), 0.001f, 1f);
            var lowPercent = Mathf.Clamp(SanitizeFinite(exposure.autoLowPercent.value, BurtPhysicalExposureSettings.DefaultAutoLowPercent), 0f, 100f);
            var highPercent = Mathf.Clamp(SanitizeFinite(exposure.autoHighPercent.value, BurtPhysicalExposureSettings.DefaultAutoHighPercent), 0f, 100f);
            ...
            var histogramMinEV100 = Mathf.Clamp(SanitizeFinite(exposure.autoHistogramMinEV100.value, BurtPhysicalExposureSettings.DefaultAutoHistogramMinEv100), -16f, 24f);
            var histogramMaxEV100 = Mathf.Clamp(SanitizeFinite(exposure.autoHistogramMaxEV100.value, BurtPhysicalExposureSettings.DefaultAutoHistogramMaxEv100), -16f, 24f);
        ...
        private static void CacheAutoParameters(CameraExposureState state, BurtPhysicalExposureSettings settings)
        {
            if (state == null)
            {
                return;
        ...
        {
            return frame >= 0 ? Mathf.Max(0, Time.frameCount - frame) : BurtPhysicalExposureSettings.DefaultAutoFrameAge;
        }
        ...
        private static bool IsReliableAverageLogLuminance(BurtExposureMode mode, float averageLogLuminance)
        {
            ...
            return mode != BurtExposureMode.Automatic || averageLogLuminance >= MinimumReliableBasicAverageLogLuminance;
        }

        private static bool IsAutomaticMode(BurtExposureMode mode)
        {
            return mode == BurtExposureMode.Automatic || mode == BurtExposureMode.AutomaticHistogram;
        }
