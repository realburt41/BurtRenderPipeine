    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            GBufferFragmentOutput FragGBuffer(GBufferVaryings input, fixed facing : VFACE)
            {
                ...
                // Hair deferred lighting needs strand direction. Without a dedicated GBuffer slot, store mesh tangent in GBuffer1.rg.
        ...
        Pass
        {
            ...
            // Includes BurtRP surface/input data structures used by the Hair forward pass.
            #include "Assets/BurtRP/Runtime/Shaders/ShaderLibrary/Material/BurtInput.hlsl"
            ...
            // 引入 BurtRP Hair 统一材质 CBUFFER，Forward pass 直接使用同一份 SRP Batcher 字段布局。
            #include "Assets/BurtRP/Runtime/Shaders/ShaderLibrary/Material/BurtHairProperties.hlsl"
            ...
            // Computes BurtRP's current minimal Hair model through ShaderLibrary helpers.
            float4 Frag(Varyings input, fixed facing : VFACE) : SV_Target
            {
                ...
                // 采样法线贴图并把切线空间法线转换成世界空间法线；Hair lighting 当前主要使用 strand direction，这里保留给 debug。
                ...
                // Standalone Hair uses mesh tangent as strand direction so Forward and Deferred follow the same lobe orientation.
                float strandDirectionSign = lerp(1.0f, -1.0f, saturate(_HairTangentFlip));
                float3 strandDirectionWS = BurtSafeNormalize(input.tangentWS.xyz * strandDirectionSign);
                ...
                debugData.additionalUnshadowedColor = BurtEvaluateAdditionalLightingUnshadowedDebug(shadingSurfaceData, strandDirectionWS, viewDirectionWS, input.positionWS);
                ...
                // 写入 GBuffer 解码后的 Hair strand direction；Hair 当前复用 normalWS 字段承载向量槽。
                debugData.gbufferNormalWS = BurtGetHairStrandDirectionWS(debugDecodedGBufferData);
