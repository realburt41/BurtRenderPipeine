    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Frag(Varyings input) : SV_Target
            {
                ...
                if (debugMode == 2)
                {
                    // 返回 oct normal.rg、metallic.b、smoothness.a 的原始编码结果。
                ...
                if (debugMode == 5)
                {
                    // 返回法线方向彩色图，用来检查 oct 解码和法线贴图方向。
                    return BurtDebugNormal(gbufferData.normalWS);
                }
                ...
                {
                    // 返回 metallic 灰度图。
                    return BurtDebugScalar(gbufferData.metallic);
                }
