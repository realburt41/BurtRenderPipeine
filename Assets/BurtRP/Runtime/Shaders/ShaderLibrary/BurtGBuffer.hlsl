...
#include "BurtBRDF.hlsl" // 引入 BurtSurfaceData、PBR 准备结构和 XRender 风格 reflectance/F0/roughness 工具。

// GBuffer0 约定：rgb = baseColor，a = occlusion；baseColor 保持材质基础色，不预乘灯光或能量项。
// GBuffer1 约定：rg = octahedron normalWS，b = metallic，a = smoothness；Deferred shading 再由 smoothness 还原 perceptual roughness。
...
struct BurtGBufferData
{
    ...
    // 保存金属度，Deferred shading 用它和 baseColor、reflectance 一起重建 F0。
    float metallic;
...
struct BurtEncodedGBuffer
{
    ...
    // GBuffer1：octa normalWS.rg + metallic + smoothness。
    float4 gbuffer1;
...
// 把世界空间法线编码成两个 0..1 通道；Deferred GBuffer1.rg 使用这个结果。
...
// 从 GBuffer1.rg 解码世界空间法线；Deferred lighting pass 应使用这个结果生成 BurtPBRGeometryData。
...
// 从 Forward 已经算好的 SurfaceData / normal / emission 生成待编码的 GBuffer 数据。
...
BurtEncodedGBuffer BurtEncodeGBuffer(BurtGBufferData data)
{
    ...
    // GBuffer1 保存压缩法线、金属度和光滑度；法线使用 octahedron 两通道编码。
    encoded.gbuffer1 = float4(BurtEncodeNormalWSForGBuffer(data.normalWS), saturate(data.metallic), saturate(data.smoothness));
...
BurtGBufferData BurtDecodeGBuffer(BurtEncodedGBuffer encoded)
{
    ...
    // GBuffer1：还原世界空间法线、金属度和光滑度。
    data.normalWS = BurtDecodeNormalWSFromGBuffer(encoded.gbuffer1.rg);
    data.metallic = saturate(encoded.gbuffer1.b);
...
BurtPBRMaterialData BurtPreparePBRMaterialData(BurtGBufferData gbufferData)
{
    ...
    // 拷贝 GBuffer 保存的材质输入语义。
    materialData.baseColor = gbufferData.baseColor;
    materialData.metallic = gbufferData.metallic;
...
BurtPBRGeometryData BurtPreparePBRGeometryData(BurtGBufferData gbufferData, float3 viewDirectionWS)
{
    ...
    return BurtPreparePBRGeometryData(gbufferData.normalWS, viewDirectionWS);
}
