...
#include "BurtGBuffer.hlsl" // 引入 BurtEncodedGBuffer / BurtGBufferData，以及 GBuffer 编解码函数。
...
{
    // 从 CameraDepth 读取当前像素的原始硬件深度。
    rawDepth = BurtSampleDeferredRawDepth(screenUV);

    // 用逆 ViewProjection 重建世界坐标，对应 XRender Deferred 的 PosData.PositionWS。
    positionWS = BurtReconstructDeferredPositionWS(screenUV, rawDepth);

    // 从表面点指向相机，保持和 Forward pass 的 viewDirectionWS 方向一致。
